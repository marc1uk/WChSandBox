{
  ...
  TH1D* Lmu = new TH1D("lm","lm",200,-100.,5000.);
  ...
  for(int i=0; i<40; i++){
    ...
    for(int k=0; k<nphot; k++){
     ...
     //       cout<<i<<" "<<k<<endl;
      ...
      std::cout << phot_hit[k] << " " << phot_tEnd[k] << " " << phot_isScat[k] << std::endl;     
 
      //if(phot_hit[k]==1 && phot_tEnd[k]<30. && phot_isScat[k]==0){
      if(phot_hit[k]==0 && phot_tEnd[k]<30. && phot_isScat[k]==0){
	...
	double vtxdistance = sqrt(dxs+dys+dzs);
        std::cout << dxs << " " << dys << " " << dzs << " " << vtxdistance << std::endl;
