...
static WCSimWaterModel* fgWaterModel = 0;

WCSimWaterModel* WCSimWaterModel::Instance()
{
  if( !fgWaterModel ){
    fgWaterModel = new WCSimWaterModel();
  }
...
/*WCSimWaterModel* WCSimWaterModel::Instance(Int_t op1, Int_t op2, Int_t op3)
{
  if( !fgWaterModel ){
    fgWaterModel = new WCSimWaterModel(op1, op2, op3);
  }
...
/*WCSimWaterModel::WCSimWaterModel(Int_t op1) 
{
    fOP = op1;
}*/
...
Double_t WCSimWaterModel::evalGraphs(Double_t flmda,char paramType) {
    ...
    if (!(paramType == '1' || paramType == '2' || paramType == '3' || paramType == '4' || paramType == '5')) 
    	std::cout << "Error in evaluating the TGraph in WCSimWaterModel........." << std::endl;    
...
/*Double_t WCSimWaterModel::N_Index_d(Double_t dist)
{
    Double_t norm = 0.0;
...
Double_t WCSimWaterModel::N_Index(Double_t flmda, Double_t Temp)
{
    r5=999.95; r1=-3.983; r2=301.797; r3=522528.9; r4=69.349; 
    a0=0.2439; a1=9.5352*0.001; a2=3.6436*-0.001; a3=2.6567*0.0001; a4=1.5919*0.001; a5=2.4573*0.001; a6=0.8975; a7=1.6307*-0.01; lambdauv=0.2292; lambdair=5.4329; 
    rho = r5*(1-((Temp+r1)*(Temp+r1)*(Temp+r2))/(r3*(Temp+r4))); //water density From: http://iopscience.iop.org/0026-1394/38/4/3/pdf/0026-1394_38_4_3.pdf
...
{   
    ...
    if( fopv==1 ) { 
      C=WCSimParameters::SpeedOfLight(); N=N_Index(flmda);
...
Double_t WCSimWaterModel::VgWCSim(Double_t flmda)
{
    if( fopv==0 ){ fvelo = fVeloLg->Eval(flmda,0,"S"); }
    if( fopv==1 ) { 
      C=WCSimParameters::SpeedOfLight(); N=N_Index(flmda);
      dndl = ((N_Index(flmda + 0.001)) - (N_Index(flmda - 0.001)))/0.002;
      fvelo = C/(N - flmda*dndl);// cm/ns
    }
    return fvelo; 
}

Double_t WCSimWaterModel::TimeMu(Double_t mudist)
{
   Double_t tmu = (1195.82 - sqrt(4*mudist*mudist - 4800.0*mudist + 1430000))/60.0;
   return tmu;
}

Double_t WCSimWaterModel::ChereAngle(Double_t mudist)
{
   Double_t tmu = (1195.82 - sqrt(4*mudist*mudist - 4800.0*mudist + 1430000))/60.0;
   return tmu;
}
...
void WCSimWaterModel::SetAbsTable(TGraph* abslg)
{ 
    fAbsLg = abslg;
}
...
//calculating the attenuating factor, the output spectrum when the input spectrum is uniform.
  ...
  Double_t WCSimWaterModel::Atten(Double_t flmda, Double_t fdist)
  {
...
void WCSimWaterModel::SetQETable(TGraph* mytg)
{
   fQEg = mytg;
}

Double_t WCSimWaterModel::QE(Double_t flmda)
{
...
Double_t WCSimWaterModel::FinlSpect(Double_t flmda, Double_t fdist)
{
    //Double_t norm2 = 0.0;
...
WCSimWaterModel::waterM WCSimWaterModel::getParamsWM(Double_t v, Double_t initi, Double_t absl, Double_t sctl, Double_t qe, Double_t fdist)
{
WCSimWaterModel::waterM waterParams;
