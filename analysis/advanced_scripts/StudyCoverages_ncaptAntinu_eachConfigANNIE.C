...
//  TString genfilename("../../generatorcardfile.root");
  TString filename("/vols/t2k03/hyperk/output/sim/ANNIE/FullEvent.root");
  TString genfilename("/vols/t2k03/hyperk/output/sim/ANNIE/generatorcardfile.root");
  ...
  TH1D *nLightAntinu1 = new TH1D("nLightAntinu1","Number of photons which reach a photosensor for a single Gd-captured neutron.",20,0,200);
  TH1D *nLightAntinu2 = new TH1D("nLightAntinu2","Number of photons which reach a photosensor for a single Gd-captured neutron.",20,0,200);
  TH1D *nLightAntinu3 = new TH1D("nLightAntinu3","Number of photons which reach a photosensor for a single Gd-captured neutron.",20,0,200);
  TH1D *nLightAntinu4 = new TH1D("nLightAntinu4","Number of photons which reach a photosensor for a single Gd-captured neutron.",20,0,200);
  TH1D *nLightAntinu5 = new TH1D("nLightAntinu5","Number of photons which reach a photosensor for a single Gd-captured neutron.",20,0,200);
  ...
  for(int config = 1; config<6; config++){  
    ...
    if(opt==1){
    // order of the walls : top, bottom, right, left, downstream, upstream
      if(config==1){
        for(int ii=1;ii<7;ii++){
          if(ii==1 || ii==2){
            sbPMT->SetWallConfiguration(ii,1,shapePMT,203.2,7,7,QE_PMT,shapeLAPPD,sizeLAPPD,0,0,QE_LAPPD);
          }
          else{
            sbPMT->SetWallConfiguration(ii,1,shapePMT,203.2,0,0,QE_PMT,shapeLAPPD,sizeLAPPD,0,0,QE_LAPPD);
	  }
        }
      }
      if(config==2){
        for(int ii=1;ii<7;ii++){
          if(ii==5 || ii==6){
            ...
            sbPMT->SetWallConfiguration(ii,1,shapePMT,203.2,5,5,QE_PMT,shapeLAPPD,sizeLAPPD,0,0,QE_LAPPD);
          }
        }
      }
      if(config==3){
        for(int ii=1;ii<7;ii++){
          if(ii==1 || ii==2){
            sbPMT->SetWallConfiguration(ii,1,shapePMT,203.2,10,10,QE_PMT,shapeLAPPD,sizeLAPPD,0,0,QE_LAPPD);
          }
          else{
            sbPMT->SetWallConfiguration(ii,1,shapePMT,203.2,0,0,QE_PMT,shapeLAPPD,sizeLAPPD,0,0,QE_LAPPD);
          }
      ...
      if(config==5){
        for(int ii=1;ii<7;ii++){
          if(ii==5){
            sbPMT->SetWallConfiguration(ii,1,shapePMT,203.2,0,0,QE_PMT,shapeLAPPD,sizeLAPPD,3,3,QE_LAPPD);
          }
          if(ii==6){
            sbPMT->SetWallConfiguration(ii,1,shapePMT,203.2,0,0,QE_PMT,shapeLAPPD,sizeLAPPD,2,2,QE_LAPPD);
          }
          else{
            sbPMT->SetWallConfiguration(ii,1,shapePMT,203.2,0,0,QE_PMT,shapeLAPPD,sizeLAPPD,0,0,QE_LAPPD);
          }
...
//          if( (k%10000==0) && (i%10==0) ) cout<<k<<" photons out of "<<nphot<<endl;  
  ...
  nLightAntinu2->SetLineColor(2);
  nLightAntinu3->SetLineColor(3);
  nLightAntinu4->SetLineColor(4);

  nLightAntinu1->GetXaxis()->SetTitle("Number of photons");
  nLightAntinu1->GetYaxis()->SetTitle("Number of events");
  nLightAntinu2->GetXaxis()->SetTitle("Number of photons");
  nLightAntinu2->GetYaxis()->SetTitle("Number of events");
  nLightAntinu3->GetXaxis()->SetTitle("Number of photons");
  nLightAntinu3->GetYaxis()->SetTitle("Number of events");
  nLightAntinu4->GetXaxis()->SetTitle("Number of photons");
  nLightAntinu4->GetYaxis()->SetTitle("Number of events");
  nLightAntinu5->GetXaxis()->SetTitle("Number of photons");
  nLightAntinu5->GetYaxis()->SetTitle("Number of events");
  ...
  TLegend *legend = new TLegend(0.40,0.55,0.86,0.86);
  legend->AddEntry(nLightAntinu1,"7 by 7 PMTs top, bottom","l");
  legend->AddEntry(nLightAntinu2,"5 by 5 PMTs top, bottom, left, right","l");
  legend->AddEntry(nLightAntinu3,"10 by 10 PMTs top, bottom","l");
  legend->AddEntry(nLightAntinu4,"7 by 7 PMTs top, bottom, left, right","l");
  legend->Draw();

  new TCanvas("canvas1","canvas1");
  nLightAntinu1->Draw();
  TLegend *legend = new TLegend(0.40,0.75,0.86,0.86);
  legend->AddEntry(nLightAntinu1,"7 by 7 PMTs top, bottom","l");
  legend->Draw();
  ...
  new TCanvas("canvas3","canvas3");
  nLightAntinu3->Draw();
  TLegend *legend = new TLegend(0.40,0.75,0.86,0.86);
  legend->AddEntry(nLightAntinu3,"10 by 10 PMTs top, bottom","l");
  legend->Draw();

  new TCanvas("canvas4","canvas4");
  nLightAntinu4->Draw();
  TLegend *legend = new TLegend(0.40,0.75,0.86,0.86);
  legend->AddEntry(nLightAntinu4,"7 by 7 PMTs top, bottom, left, right","l");
  legend->Draw();

  new TCanvas("canvas5","canvas5");
  nLightAntinu5->Draw();
  TLegend *legend = new TLegend(0.30,0.75,0.86,0.86);
  legend->AddEntry(nLightAntinu5,"3 by 3 LAPPDs downstream and 2 by 2 LAPPDs upstream","l");
  legend->Draw();
