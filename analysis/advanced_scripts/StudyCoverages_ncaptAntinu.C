...
//  TString genfilename("../../generatorcardfile.root");
  ...
  TH1D *nLightAntinu = new TH1D("nLightAntinu","Number of photons which reach a photosensor for a single Gd-captured neutron.",50,0,500);
  TH1D *nCaptAntinu_CCQE = new TH1D("nCaptAntinu_CCQE","Number of neutrons captured in CCQE mode",31,-0.5,30.5);
  TH1D *nCaptAntinu_nonCCQE = new TH1D("nCaptAntinu_nonCCQE","Number of neutrons captured in non-CCQE mode",31,-0.5,30.5);
  TH1D *nCaptAntinu_NC = new TH1D("nCaptAntinu_NC","Number of neutrons captured in NC mode",31,-0.5,30.5);
 ...
 // Loop over number of entries
  for(int i=0; i<nentries; i++){
    ...
    if(fabs(mmode)==1){
      nCaptAntinu_CCQE->Fill(ncapturecount);
    }
    else{
      if((fabs(mmode)>=2 && fabs(mmode)<=30) || (fabs(mmode)==0 && whichFile==1)){
        nCaptAntinu_nonCCQE->Fill(ncapturecount);
      }
      else{
        if(fabs(mmode)>=31 && fabs(mmode)<=60){
          nCaptAntinu_NC->Fill(ncapturecount);
        }
      }
    }
...
//        if( (k%10000==0) && (i%10==0) ) cout<<k<<" photons out of "<<nphot<<endl;  
    ...
    if (nphotbyNeutron != 0) nLightAntinu->Fill(nphotbyNeutron);
  ...
  nLightAntinu->SetLineStyle(4);
  nLightAntinu->Draw();

  new TCanvas("canvas22","canvas22");
  nCaptAntinu_CCQE->SetLineStyle(4);
  nCaptAntinu_CCQE->Draw();
  new TCanvas("canvas32","canvas32");
  nCaptAntinu_nonCCQE->SetLineStyle(4);
  nCaptAntinu_nonCCQE->Draw();
  new TCanvas("canvas42","canvas42");
  nCaptAntinu_NC->SetLineStyle(4);
  nCaptAntinu_NC->Draw();
