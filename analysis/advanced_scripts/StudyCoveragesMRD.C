...
//  TString genfilename("../../generatorcardfile.root");
  ...
  TH1D *histMu1 = new TH1D("histMu1","Energy spectrum of muons that reach MRD",50,0,3000);
  TH1D *histMu2 = new TH1D("histMu2","Energy spectrum of muons that reach MRD in configuration 2",50,0,3000);
  TH1D *histMu3 = new TH1D("histMu3","Energy spectrum of muons that reach MRD in configuration 3",50,0,3000);
  TH1D *histMu4 = new TH1D("histMu4","Energy spectrum of muons that reach MRD in configuration 4",50,0,3000);
  TH1D *histNu1 = new TH1D("histNu1","Energy spectrum of neutrinos which create muons that reach MRD",50,0,3000);
  TH1D *histNu2 = new TH1D("histNu2","Energy spectrum of muons neutrinos which create that reach MRD in configuration 2",50,0,3000);
  TH1D *histNu3 = new TH1D("histNu3","Energy spectrum of muons neutrinos which create that reach MRD in configuration 3",50,0,3000);
  TH1D *histNu4 = new TH1D("histNu4","Energy spectrum of muons neutrinos which create that reach MRD in configuration 4",50,0,3000);
  ...
  for(int i=0; i<nentries; i++){
	...
	for(int barrelConfig=1; barrelConfig<5; barrelConfig++){
	  ...
	  if((part_zEnd[l]<zEndBarrel[barrelConfig] && posfR<-649.*sin(fabs(posfTheta))+Rdet*(1.118)+500. 
	    ...
	    if(barrelConfig==1){
	      histMu1->Fill(partEnergy);
	      histNu1->Fill(neutrino_E);
	    }
	    if(barrelConfig==2){
              histMu2->Fill(partEnergy);
              histNu2->Fill(neutrino_E);
	    }
	    if(barrelConfig==3){
              histMu3->Fill(partEnergy);
              histNu3->Fill(neutrino_E);
	    }
	    if(barrelConfig==4){
              histMu4->Fill(partEnergy);
              histNu4->Fill(neutrino_E);
	    }
	  }
        }
      }
  ...
  new TCanvas("c1","c1");
  histMu1->SetLineColor(1);
  histMu1->Draw();
  histMu1->GetXaxis()->SetTitle("Energy of muons");
  histMu2->SetLineColor(2);
  histMu2->Draw("same");
  histMu3->SetLineColor(3);
  histMu3->Draw("same");
  histMu4->SetLineColor(4);
  histMu4->Draw("same");

  TLegend *legend = new TLegend(0.45,0.55,0.76,0.82);
  legend->AddEntry(histMu1,"Configuration 0 and 1","l");
  legend->AddEntry(histMu2,"Configuration 2","l");
  legend->AddEntry(histMu3,"Configuration 3","l");
  legend->AddEntry(histMu4,"Configuration 4","l");
  legend->Draw();

  new TCanvas("c2","c2");
  histNu1->SetLineColor(1);
  histNu1->Draw();
  histNu1->GetXaxis()->SetTitle("Energy of neutrinos");
  histNu2->SetLineColor(2);
  histNu2->Draw("same");
  histNu3->SetLineColor(3);
  histNu3->Draw("same");
  histNu4->SetLineColor(4);
  histNu4->Draw("same");
