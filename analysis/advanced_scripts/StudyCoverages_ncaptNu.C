...
//    TString genfilename("../../generatorcardfile.root");
    TString filename("/vols/t2k03/hyperk/output/sim/genie/genie/FullEvent_nu_*.root");
    TString genfilename("/vols/t2k03/hyperk/output/sim/genie/genie/generatorcardfile_nu_*.root");
  ...
  TH1D *nLightNu = new TH1D("nLightNu","Number of photons which reach a photosensor for a single Gd-captured neutron.",50,0,500);
  TH1D *nCaptNu_CCQE = new TH1D("nCaptNu_CCQE","Number of neutrons captured in CCQE mode",31,-0.5,30.5);
  TH1D *nCaptNu_nonCCQE = new TH1D("nCaptNu_nonCCQE","Number of neutrons captured in non-CCQE mode",31,-0.5,30.5);
  TH1D *nCaptNu_NC = new TH1D("nCaptNu_NC","Number of neutrons captured in NC mode",31,-0.5,30.5);
 ...
 // Loop over number of entries
  for(int i=0; i<nentries; i++){
    ...
    Double_t mmode = mTR->get_genmode();
    ...
    if(fabs(mmode)==1){
      nCaptNu_CCQE->Fill(ncapturecount);
    }
    else{
      if((fabs(mmode)>=2 && fabs(mmode)<=30) || (fabs(mmode)==0 && whichFile==1)){
        nCaptNu_nonCCQE->Fill(ncapturecount);
      }
      else{
        if(fabs(mmode)>=31 && fabs(mmode)<=60){
          nCaptNu_NC->Fill(ncapturecount);
        }
      }
    }
...
//        if( (k%10000==0) && (i%10==0) ) cout<<k<<" photons out of "<<nphot<<endl;  
    ...
    if (nphotbyNeutron != 0) nLightNu->Fill(nphotbyNeutron);
  ...
  nLightNu->GetXaxis()->SetTitle("Number of photons");
  nLightNu->Draw();

  new TCanvas("canvas2","canvas2");
  nCaptNu_CCQE->Draw();
  nCaptNu_CCQE->GetXaxis()->SetTitle("Number of neutrons captured");
  new TCanvas("canvas3","canvas3");
  nCaptNu_nonCCQE->Draw();
  nCaptNu_nonCCQE->GetXaxis()->SetTitle("Number of neutrons captured");
  new TCanvas("canvas4","canvas4");
  nCaptNu_NC->Draw();
  nCaptNu_NC->GetXaxis()->SetTitle("Number of neutrons captured");
