...
class mrdPMThit : public G4VHit
{
  public:
 
    //mrdPMThit();
    mrdPMThit(const G4Step* aStep);
    virtual ~mrdPMThit();
    mrdPMThit(const mrdPMThit &right);

    const mrdPMThit& operator=(const mrdPMThit &right);
    G4int operator==(const mrdPMThit &right) const;
...
typedef G4THitsCollection<mrdPMThit> mrdPMThitsCollection;

extern G4Allocator<mrdPMThit> mrdPMThitAllocator;

inline void* mrdPMThit::operator new(size_t){
    return (void *) mrdPMThitAllocator.MallocSingle();
}

inline void mrdPMThit::operator delete(void *aHit){
  mrdPMThitAllocator.FreeSingle((mrdPMThit*) aHit);
}
