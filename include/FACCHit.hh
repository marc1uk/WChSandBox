...
class FACCHit : public G4VHit
{
  public:

      FACCHit();
      FACCHit(G4Step* aStep);
      ~FACCHit();
      //FACCHit(const FACCHit &right);
      //const FACCHit& operator=(const FACCHit &right); //same as implicit definition
      int operator==(const FACCHit &right) const;
...
typedef G4THitsCollection<FACCHit> FACCHitsCollection;

extern G4Allocator<FACCHit> FACCHitAllocator;

inline void* FACCHit::operator new(size_t)
{
  return (void *) FACCHitAllocator.MallocSingle();
}

inline void FACCHit::operator delete(void *aHit)
{
  FACCHitAllocator.FreeSingle((FACCHit*) aHit);
}

inline G4int FACCHit::operator==
       (const FACCHit& right) const 
{
   return (this==&right) ? 1 : 0; 
}

/*inline const SBsimFACCHit& SBsimFACCHit::operator= (const SBsimFACCHit& right) {
