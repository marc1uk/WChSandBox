  ...
  :myDetector(myDC)
{
  ...
  useMulineEvt = true; //true;
  useOpticalPhotEvt = false; //true;
  useNormalEvt = false;
  
  // optical photon gun for testing MRD optical response
  _gun = new G4ParticleGun(G4OpticalPhoton::Definition());
  _gun->SetParticlePosition(G4ThreeVector(0.*m,0.*m,2.022*m));	// layers are only MM not m! - this is in first scint panel.
  _gun->SetParticleMomentumDirection(G4ThreeVector(1.0,0.,0.));	// fire in x direction
