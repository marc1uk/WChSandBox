  ...
  :myDetector(myDC)
{
  ...
  ttttf = new TFile("generatorcardfile.root","RECREATE");
  tcardfile = new TTree("tcardfile","tcardfile");

  const int kmaxtrk=100;
  const int kmaxMRDhits=100;
...
void WCLitePrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
  ...
  //  - Default for vtxchooser switched from 2 to 1 
  //    (i.e., from random vertices to reading from card file)
  // 0 means all vertices forced to origin
  ...
  { 
	...
	token = readInLine(inputFile, lineSize, inBuf);
	...
	else   // normal parsing begins here
	  {
		  ...
		  if( (atoi(token[6]))==0 && (atoi(token[1])!=8016) )
		    {
		      ...
		      ntrks++;
		      if(ntrks>100) G4cout<<"ntrks exceeded array size (100)!!"<<G4endl;
	    ...
	    for(int pgi=0; pgi<idvec.size(); pgi++){
	             
	      mpid[pgi] = (G4int) idvec.at(pgi);
		...
		particleGun->GetParticleDefinition()->GetPDGMass();
	      ...
	      mKE[pgi] = (G4double) ekin;	     	      		      

	      G4ThreeVector idir = dirvec.at(pgi);
	      
	      mpx[pgi] = (G4double) idir.x();
	      mpy[pgi] = (G4double) idir.y();
	      mpz[pgi] = (G4double) idir.z();
	      
	      //std::cout<<"4-momenta: "<<mpx[pgi]<<" "<<mpy[pgi]<<" "<<mpz[pgi]<<" "<<ntrks<<std::endl;
		      ...
		      ((token[0] == "V")||(token[0] == "H")) )
		{
		  ...
		  mMRDhitEdep[layercount]= (G4double) atof(token[2]);
		  mMRDhitEchdep[layercount]=(G4double) atof(token[3]);
		  ...
		  layercount++;
		  if(layercount>100) G4cout<<"nmrdlayers exceeded array size (100)!!"<<G4endl;
		}
