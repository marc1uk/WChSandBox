...
WCLitePrimaryGeneratorAction::WCLitePrimaryGeneratorAction(WCLiteDetectorConstruction* myDC) : myDetector(myDC), useNuanceTextFormat(true)
{
  ...
  useMuonEvt = false;
  useNormalEvt = false;
  useBeamEvt = false;	// true for new geant4 dirt simulations
...
//	rdrkEp = &rdrkE;
  ...
  if(useNeutronEvt){
	  // neutron gun for testing NCV response
	  NeutronGun = new G4ParticleGun(particleTable->FindParticle(particleName="neutron"));
	  NeutronGun->SetParticlePosition(G4ThreeVector(0.*cm,0.*cm,1.670*m)); 			// just inside ncv in z dirn
	  NeutronGun->SetParticleMomentumDirection(G4ThreeVector(0.,0.,1.));		// fire in z direction
	  NeutronGun->SetParticleEnergy(1*MeV);	// Gd resonance forest should mean it has significant capture x-sec even above thermal E
  }
  
  if(useGeantinoEvt){
	  // geantino gun for testing geometry (note geometry has changed! numbers outdated!)
	  ...
	  GeantinoGun->SetParticlePosition(G4ThreeVector(3.0*cm, 3.0*cm,2.0*cm));		// in veto paddle	//-1.545m z
  ...
  if(useMuonEvt){
	  G4cout<<"using muon gun."<<G4endl;
	  // high energy muon gun for testing geometry
	  MuonGun = new G4ParticleGun();
	  MuonGun->SetParticleDefinition(particleTable->FindParticle(particleName="mu-"));
	  //MuonGun->SetParticlePosition(G4ThreeVector(3.0*cm, 3.0*cm, 0.0*cm));
	  //MuonGun->SetParticleMomentumDirection( G4ThreeVector(0.0,0.0,1.0) );		// needs to be off-centre due to inter-paddle gaps
	  MuonGun->SetParticlePosition(G4ThreeVector(3.0*cm, 3.0*cm,(-1.545)*m));		// in veto paddle
	  MuonGun->SetParticleMomentumDirection( G4ThreeVector(0.0,0.0,1.0) );			// straight through whole detector
	  MuonGun->SetParticleEnergy( 10.0*GeV );
	  MuonGun->SetParticleTime( 0.0*ns );
  }
...
WCLitePrimaryGeneratorAction::~WCLitePrimaryGeneratorAction()
{
  ...
  if(useNeutronEvt){delete NeutronGun;}
  if(useGeantinoEvt) {delete GeantinoGun;}
  if(useMuonEvt){delete MuonGun;}
...
//	    G4double npl=-555.;
  ...
  else if (useGeantinoEvt||useMuonEvt||useOpticalPhotEvt||useNeutronEvt)
  {	// manual particle firing
    if(useGeantinoEvt){GeantinoGun->GeneratePrimaryVertex(anEvent);}
    else if(useMuonEvt) {MuonGun->GeneratePrimaryVertex(anEvent);}
    else if(useNeutronEvt) {NeutronGun->GeneratePrimaryVertex(anEvent);}
    else if(useOpticalPhotEvt) {OpPhotonGun->GeneratePrimaryVertex(anEvent);}
    else if(useLaserEvt) {MyGPS->GeneratePrimaryVertex(anEvent);}
    else if(useNormalEvt) {particleGun->GeneratePrimaryVertex(anEvent);}
