...
WCSimWCSD::WCSimWCSD(G4String CollectionName, G4String name,WCLiteDetectorConstruction* myDet)
...
{ 
  ...
  WCLiteTrackInformation* trackinfo 
    = (WCLiteTrackInformation*)(aStep->GetTrack()->GetUserInformation());
  G4int primParentID;
  if (trackinfo)
    primParentID = 0; //trackinfo->GetPrimaryParentID();		// marcus: currently not implemented in WCLite.
  ...
  // See WCLiteDetectorConstruction::DescribeAndRegisterPMT() for matching
  // tag construction.
...
//  G4cout << tubeTag.str() << G4endl;
  ...
  G4int replicaNumber = WCLiteDetectorConstruction::GetTubeID(tubeTag.str());
