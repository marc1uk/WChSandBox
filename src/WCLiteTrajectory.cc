...
void WCLiteTrajectory::AppendStep(const G4Step* aStep)
{
  ...
  // check if this step puts the track in the MRD
  ...
  // check for possible superfluous scintillation
  ...
  G4double depositedenergythisstep = aStep->GetTotalEnergyDeposit();
	...
	if(sdetectorname=="MuonRangeDetector"){
		...
		// Then record those things that needed it:
		// ensure passage through MRD is noted if needed
		if(sdetectorname=="MuonRangeDetector"){
