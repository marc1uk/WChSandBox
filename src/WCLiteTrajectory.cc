...
void WCLiteTrajectory::AppendStep(const G4Step* aStep)
{
  ...
  if(aTrack->GetDefinition()->GetParticleName()=="neutron"){
 	...
 	G4int numpoints=0;
 	WCLiteTrajectory* trajec=0;
 	WCLiteTrackInformation* info = (WCLiteTrackInformation*)(aTrack->GetUserInformation());
 	if(info !=0){ 
 		trajec = info->GetParentTrajectory();
 		numpoints = trajec->GetPointEntries();
 	}
 									
  	//G4cout << "Appending point (" << pntx << ", " <<  pnty << ", " << pntz << ") from trackID " << trackiddd;
  	//G4cout << " to trajectory " << trajec << " which currently has " << numpoints << " points." << G4endl;
  }
