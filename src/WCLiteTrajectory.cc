...
WCLiteTrajectory::WCLiteTrajectory()
  :  positionRecord(0), fParentID(0), fTrackID(0), PDGEncoding( 0 ), PDGCharge(0.0), 
     ...
     ncvEntryPos( G4ThreeVector() ), ncvEntryTime(0.), ncvExitPos( G4ThreeVector() ), ncvExitTime(0.), lastsdetectorname("")
{;}
...
WCLiteTrajectory::WCLiteTrajectory(const G4Track* aTrack)
{
 ...
 // ================================================
  ...
  if(theLocation=="NCVliquid_phys"){
    ...
    lastsdetectorname = "NeutronCaptureVolume";
  }
...
void WCLiteTrajectory::AppendStep(const G4Step* aStep)
{
  ...
  if(sensdet!=0){
    ...
    sdetectorname = "";
		...
		if(sdetectorname=="NeutronCaptureVolume"||(lastsdetectorname=="NeutronCaptureVolume"&&sdetectorname!="NeutronCaptureVolume")){
...
//			}
			if(sdetectorname=="NeutronCaptureVolume"){
				...
				userinfo->SetNCVexitPos(aStep->GetPostStepPoint()->GetPosition());
				userinfo->SetNCVexitTime(aStep->GetPostStepPoint()->GetGlobalTime());
			}
		}
