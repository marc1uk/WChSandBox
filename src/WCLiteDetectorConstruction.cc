...
std::map<int, G4Transform3D> WCLiteDetectorConstruction::tubeIDMap;
//std::map<int, cyl_location>  WCLiteDetectorConstruction::tubeCylLocation;
hash_map<std::string, int, hash<std::string> > 
WCLiteDetectorConstruction::tubeLocationMap;

// ***********/WCSim PMT integration
// =================================

G4double tankouterRadius= 1.524*m;	// 120" exactly (TSW blueprint) = 3.048m diameter
G4double tankhy = 1.98*m;		// 13ft exactly (TSW blueprint) = 3.96m tall; hz is HALF height
G4double tankzoffset = 15.70*cm;	//15.70*cm
G4double tankyoffset = 144.64875*mm;
...
WCLiteDetectorConstruction::WCLiteDetectorConstruction(G4String gdmlfname) // skipping arg from SciBooNE integration
{
  ...
  //-------------------------------------------------------------
  // WCSim PMT parameters: default method for implementing PMT QE
  //-------------------------------------------------------------
  SetPMT_QE_Method(1);
  //default is to use collection efficiency
  SetPMT_Coll_Eff(1);
  
  // Set the PMT details to ANNIE's, including WCPMTName and WCIDCollectionName. 
  SetANNIEGeometry();
...
// ******************************************************
// WCSim auxiliary functions for supporting PMT additions
// ======================================================
WCSimPMTObject* WCLiteDetectorConstruction::CreatePMTObject(G4String PMTType, G4String CollectionName)
{
  if (PMTType == "PMT8inch"){ // the only type of PMT in ANNIE
    WCSimPMTObject* PMT = new PMT8inch;
    WCLiteDetectorConstruction::SetPMTPointer(PMT, CollectionName);
    return PMT;
  }
}

void WCLiteDetectorConstruction::SetANNIEGeometry()
{
