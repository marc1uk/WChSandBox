		...
		*/ 
		...
		if(secondaries) { 																										// If it has any
		  ...
		  if(nSeco>0) {																												// If it has any...
		    ...
		    if (numDaughterNeutrons!=0 && (G4int)(aTrack->GetTrackStatus())==2){	// if we retrieved a daughter neutron
		      ...
		      WCLiteTrackInformation* infoNew = new WCLiteTrackInformation(aTrack);	// create a new UserInformation object
		    	...
		    	infoNew->SetSameAsParentTrackID(aTrack->GetTrackID());					// delcare this daughter the same as it's parent.
...
// for all particles produced in the mrd pass parentage information to any secondaries
	WCLiteTrackInformation* info = (WCLiteTrackInformation*)(aTrack->GetUserInformation());
	if (info!=0){		// primary particles that have not passed through the MRD may not have user info
		G4int mrdorig = info->GetmrdOriginalTrackID();											// was it this or an ancestor...?
		if(info->GetPassThruMRD()==1 || mrdorig!=0) {	  // this track or one of it's ancestors passed through the MRD				
			G4TrackVector* secondaries = fpTrackingManager->GimmeSecondaries();		// Get list of secondaries
			if(secondaries){	 																										// If it has any
				...
				if(nSeco>0) {																													// if really has any
					...
					for(size_t i=0;i<nSeco;i++){																				// for each secondary
						...
						if(secondarytrack->GetUserInformation()){														// check for user info (created above for neutrons)
							WCLiteTrackInformation* infoSeco = (WCLiteTrackInformation*)(secondarytrack->GetUserInformation());	// use if already set; just set originator track id
							infoSeco->SetmrdOriginalTrackID(mrdorig);
						} else {
							WCLiteTrackInformation* infoNew = new WCLiteTrackInformation();	// create a new UserInformation object
							infoNew->SetIsPrimaryN(false);
							infoNew->SetmrdOriginalTrackID(mrdorig);																// set originator
							secondarytrack->SetUserInformation(infoNew);												// and link it to the daughter.
						}
					}
				}
			}
	 } // else: neither this nor any ancestor tracks passed through the MRD. 
 }
 if(info!=0){	// we're at the end of processing a Track that had some useful information. If we don't want to lose it, we need to copy to Trajectory.
